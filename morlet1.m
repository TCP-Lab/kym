...
%
%--------------------------------------------------------------------------------
% Morlet Wavelet ver.1 (Approximate form)
%--------------------------------------------------------------------------------
%
%
% Function Definition
...
for jo = 1:noctave % Octave index
	...
	for jv = 0:nvoice-1 % Voice index
		...
		% Fourier transform of the Morlet wavelet (Approximate form)
		Psi = exp(-(sigma2/2)*(2*pi*freq*a - s).^2);
		% That is to say: exp(-(sigma2/2)*((freq - 1/a)^2)/(1/(2*pi*a)^2) if s=2*pi
